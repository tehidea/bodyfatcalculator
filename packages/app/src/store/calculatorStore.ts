...
/**
 * Converts input values between measurement systems.
 * Skinfold measurements are always in mm and don't need conversion.
 */
function convertInputs(
  ...
  if (from === to) return { ...inputs, gender }

  const converted: StandardizedInputs = {
    gender,
  ...
  Object.entries(inputs).forEach(([key, value]) => {
    ...
    if (typeof value === 'number') {
      converted[key] = convertMeasurement(value, conversionType, from, to)
    }
  })

...
jest.mock('expo-crypto', () => ({
	randomUUID: jest.fn(
		() => 'test-uuid-' + Math.random().toString(36).slice(2, 8),
	),
}))
...
describe('useHistoryStore', () => {
	...
	describe('addMeasurement', () => {
		...
		it('prepends to the measurements array', () => {
			...
			const first = addMeasurement(
				createMeasurementParams({ bodyFatPercentage: 20 }),
			)
			const second = addMeasurement(
				createMeasurementParams({ bodyFatPercentage: 22 }),
			)
	...
	describe('deleteMeasurement', () => {
		it('sets deletedAt to an ISO timestamp', () => {
			const record = useHistoryStore
				.getState()
				.addMeasurement(createMeasurementParams())
			...
			useHistoryStore.setState((state) => ({
				measurements: state.measurements.map((m) =>
					m.clientId === record.clientId
						? { ...m, syncedAt: '2024-01-01T00:00:00.000Z' }
						: m,
				),
			}))
	...
	describe('markSynced', () => {
		it('sets syncedAt for given clientIds', () => {
			...
			useHistoryStore
				.getState()
				.markSynced([first.clientId, second.clientId])
	...
	describe('mergeFromCloud', () => {
		...
		it('applies cloud deletions when local record is not deleted', () => {
			const localRecord = useHistoryStore
				.getState()
				.addMeasurement(createMeasurementParams())
		...
		it('sorts by measuredAt descending after merge', () => {
			...
			// Most recent first
			expect(
				new Date(measurements[0]!.measuredAt).getTime(),
			).toBeGreaterThan(new Date(measurements[1]!.measuredAt).getTime())
		})

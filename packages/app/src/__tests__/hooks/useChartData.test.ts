...
jest.mock('@bodyfat/shared/conversions', () => ({
	convertMeasurement: jest.fn(
		(value: number, type: string, from: string, to: string) => {
			if (from === to) return value
			if (type === 'weight')
				return from === 'metric' ? value * 2.20462 : value / 2.20462
			if (type === 'length')
				return from === 'metric' ? value * 0.393701 : value * 2.54
			return value
		},
	),
}))
...
describe('useChartData', () => {
	...
	describe('metric extraction', () => {
		it('weight comes from record.inputs.weight', () => {
			...
			const { result } = renderHook(() =>
				useChartData([record], 'weight', 'all', 'all', 'metric'),
			)

...
function toImperialCircumferences(
	metric: StandardizedInputs,
): StandardizedInputs {
	const imperial: StandardizedInputs = { ...metric }

	if (metric.weight != null) {
		imperial.weight = convertMeasurement(
			metric.weight as number,
			'weight',
			'metric',
			'imperial',
		)
	...
	if (metric.height != null) {
		imperial.height = convertMeasurement(
			metric.height as number,
			'length',
			'metric',
			'imperial',
		)
	...
	for (const field of lengthFields) {
		if (metric[field] != null) {
			imperial[field] = convertMeasurement(
				metric[field] as number,
				'length',
				'metric',
				'imperial',
			)
...
describe('YMCA formula', () => {
	...
	it('gives approximately the same BF% for equivalent metric and imperial inputs', () => {
		...
		expect(metricResult.bodyFatPercentage).toBeCloseTo(
			imperialResult.bodyFatPercentage,
			0,
		)
	})
	...
	it('bigger waist leads to higher BF%', () => {
		...
		expect(biggerResult.bodyFatPercentage).toBeGreaterThan(
			normalResult.bodyFatPercentage,
		)
	})
})
...
describe('Durnin & Womersley formula', () => {
	...
	it('uses skinfolds directly (no unit conversion needed)', () => {
		...
		expect(metricResult.bodyFatPercentage).toBe(
			imperialResult.bodyFatPercentage,
		)
	})
...
describe('Jackson & Pollock 3-site formula', () => {
	...
	it('uses different skinfold sites for male and female', () => {
		...
		expect(femaleOriginal.bodyFatPercentage).toBe(
			femaleModified.bodyFatPercentage,
		)
	})
})
...
describe('Jackson & Pollock 4-site formula', () => {
	...
	it('uses same 4 skinfold sites for both genders', () => {
		...
		const maleModified = jackson4Formula.calculate(
			{ ...maleMetric, chestSkinfold: 30 },
			'metric',
		)
		expect(maleOriginal.bodyFatPercentage).toBe(
			maleModified.bodyFatPercentage,
		)
	})
	...
	it('higher skinfolds lead to higher BF%', () => {
		...
		expect(thickerResult.bodyFatPercentage).toBeGreaterThan(
			normal.bodyFatPercentage,
		)
	})
})
...
describe('Parrillo formula', () => {
	...
	it('gives approximately the same BF% for equivalent metric and imperial inputs', () => {
		...
		const imperialInputs = toImperialCircumferences(maleMetric)
		const imperialResult = parrilloFormula.calculate(
			imperialInputs,
			'imperial',
		)
		expect(metricResult.bodyFatPercentage).toBeCloseTo(
